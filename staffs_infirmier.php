...
<head>
    ...
    <link rel="stylesheet" href="css/staffsInfirmiers.css">
</head>
...
<body>
    ...
    <div class="search">
        <?php if (isset($_POST['rechercher']) && !empty($_POST['rechercher'])) {
            ...
            $sql = $connexion->prepare("SELECT * FROM infirmier WHERE nom_infirmier LIKE ? OR prenom_infirmier LIKE ? ORDER BY nom_infirmier ASC");
            $sql->execute(["%$recherche%", "%$recherche%"]);
            $all_infirmiers = $sql->fetchAll();
        } else {
            $sql = $connexion->prepare("SELECT * FROM infirmier ORDER BY id_infirmier ASC");
            $sql->execute();
            $all_infirmiers = $sql->fetchAll();
        }
        ?>
